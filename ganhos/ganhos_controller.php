...
// Garantir que as tabelas 'ganhos' e 'mes_resumo_financeiro' existam
$conn->query("
...
/**
 * CORRIGIDO: Lista todos os ganhos do usuário logado.
 * Esta função agora é usada pelo front-end home.php para obter todos os dados.
 */
function listarGanhos($conn, $usuario_id) {
    try {
        ...
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            throw new Exception("Erro na preparação da query 'listarGanhos': " . $conn->error);
        }
        $stmt->bind_param('i', $usuario_id);
        if (!$stmt->execute()) {
            throw new Exception("Erro na execução da query 'listarGanhos': " . $stmt->error);
        }
        $res = $stmt->get_result();
        $retorno = [];
...
/**
 * Cria ou atualiza um ganho para o usuário logado e limpa o cache do mês.
 * @param mysqli $conn Objeto de conexão com o banco de dados.
 * @param string $funcao 'salvar' ou 'atualizar'.
 * @param int $usuario_id ID do usuário logado.
 */
function criarOuAtualizarGanho($conn, $funcao, $usuario_id) {
    try {
        ...
        if ($funcao === 'salvar' && empty($id)) {
            ...
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception("Erro na preparação da query 'salvarGanho': " . $conn->error);
            }
            ...
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception("Erro na preparação da query 'atualizarGanho': " . $conn->error);
            }
...
/**
 * Busca um ganho específico por ID para o usuário logado.
 * @param mysqli $conn Objeto de conexão com o banco de dados.
 * @param int $usuario_id ID do usuário logado.
 */
function buscarGanho($conn, $usuario_id) {
    try {
        ...
        if (!$id) {
            enviarErroJson('ID do ganho inválido.');
        }
        $sql = "SELECT id, descricao, valor, data FROM ganhos WHERE id = ? AND usuario_id = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            throw new Exception("Erro na preparação da query 'buscarGanho': " . $conn->error);
        }
        $stmt->bind_param('ii', $id, $usuario_id);
        if (!$stmt->execute()) {
            throw new Exception("Erro na execução da query 'buscarGanho': " . $stmt->error);
        }
        $res = $stmt->get_result();
        if ($res && $res->num_rows > 0) {
...
/**
 * Deleta um ganho específico por ID para o usuário logado e limpa o cache do mês.
 * @param mysqli $conn Objeto de conexão com o banco de dados.
 * @param int $usuario_id ID do usuário logado.
 */
function deletarGanho($conn, $usuario_id) {
    try {
        ...
        // Antes de deletar, buscamos a data do ganho para limpar o cache
        $sqlBuscaData = "SELECT data FROM ganhos WHERE id = ? AND usuario_id = ?";
        $stmtBuscaData = $conn->prepare($sqlBuscaData);
        $stmtBuscaData->bind_param('ii', $id, $usuario_id);
        $stmtBuscaData->execute();
        ...
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            throw new Exception("Erro na preparação da query 'deletarGanho': " . $conn->error);
        }
        $stmt->bind_param('ii', $id, $usuario_id);
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
...
/**
 * Lista todos os ganhos do mês para o usuário logado.
 * @param mysqli $conn Objeto de conexão com o banco de dados.
 * @param int $usuario_id ID do usuário logado.
 */
function ganhosMes($conn, $usuario_id) {
    try {
        ...
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            throw new Exception("Erro na preparação da query 'ganhosMes': " . $conn->error);
        }
        $stmt->bind_param('ssi', $dataInicio, $dataFim, $usuario_id);
        if (!$stmt->execute()) {
            throw new Exception("Erro na execução da query 'ganhosMes': " . $stmt->error);
        }
        $res = $stmt->get_result();
        $retorno = [];
        if ($res && $res->num_rows > 0) {
